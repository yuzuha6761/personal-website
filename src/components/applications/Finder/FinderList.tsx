import { finderItems } from './data'
import { fileIconMap, finderToolbarIcons } from './icons'
...
const disclosureIconClass = 'w-[.48rem] h-[.48rem] [filter:invert(43%)]'
const fileIconClass = 'w-4 h-4 object-contain'
const folderIconClass = `${fileIconClass} [filter:invert(56%)_sepia(93%)_saturate(1940%)_hue-rotate(175deg)_brightness(100%)_contrast(96%)]`
          ...
          >
            <span className="min-w-0 px-[.72rem] overflow-hidden text-ellipsis whitespace-nowrap flex items-center gap-[.28rem]">
              <span className="w-[.48rem] h-[.72rem] flex items-center justify-center">
                {item.expanded && <img className={disclosureIconClass} src={finderToolbarIcons.chevronRight} alt="" />}
              </span>
              <span className="relative flex-[0_0_1.14rem] h-[1.14rem] flex items-center justify-center">
                <img className={item.icon === 'folder' ? folderIconClass : fileIconClass} src={fileIconMap[item.icon]} alt="" />
                {item.icon !== 'folder' && (
